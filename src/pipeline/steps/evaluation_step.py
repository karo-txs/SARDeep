...
class Evaluation(Step):
    ...
    def calculate_means(self):
        ...
        df_general = {"model": [], "AP25": [], "AP50": [], "AP75": [],
                        ...
                        try:
                            if is_quantized is False:
                                model_map = df.loc[(df.model == model) & (df.device == device)
                                                   & (df.dataset_test == dataset)].query('is_quantized == False')
                                if not model_map.empty:
                                    self.append_values(df_general, model, model_map, is_quantized=False, approach=None,
