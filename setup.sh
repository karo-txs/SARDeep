...
if [ ! -d "$dataset_path/labels.txt" ]; then
  for i in ${labels//,/ }
    do
...
for i in ${datasets//,/ }
  do
    if [ ! -d "$dataset_path/$i/coco" ]; then

      echo "2/5 Create paths"
      mkdir "$dataset_path/$i/coco"
      mkdir "$dataset_path/$i/coco/annotations"
      mkdir "$dataset_path/$i/coco/images/"
      mkdir "$dataset_path/$i/coco/images/train2017"
      mkdir "$dataset_path/$i/coco/images/val2017"
      mkdir "$dataset_path/$i/coco/images/test2017"

      echo "2/5 Convert train dataset to coco format"
      python src/infra/scripts/voc_to_coco.py \
      --ann_dir "$dataset_path/$i/VOC2012/Annotations" \
      --ann_ids "$dataset_path/$i/VOC2012/ImageSets/Main/train.txt" \
      --labels "$dataset_path/labels.txt" \
      --output "$dataset_path/$i/coco/annotations/instances_train2017.json"

      echo "2/5 Convert val dataset to coco format"
      python src/infra/scripts/voc_to_coco.py \
      --ann_dir "$dataset_path/$i/VOC2012/Annotations" \
      --ann_ids "$dataset_path/$i/VOC2012/ImageSets/Main/val.txt" \
      --labels "$dataset_path/labels.txt" \
      --output "$dataset_path/$i/coco/annotations/instances_val2017.json"

      echo "2/5 Convert test dataset to coco format"
      python src/infra/scripts/voc_to_coco.py \
      --ann_dir "$dataset_path/$i/VOC2012/Annotations" \
      --ann_ids "$dataset_path/$i/VOC2012/ImageSets/Main/test.txt" \
      --labels "$dataset_path/labels.txt" \
      --output "$dataset_path/$i/coco/annotations/instances_test2017.json"
...
for i in ${datasets//,/ }
  do
    ...
    if [ ! -d "src/mmdetection/data/$i" ]; then
      ...
      cp -R "$dataset_path/$i" "src/mmdetection/data/"
    fi
  done
